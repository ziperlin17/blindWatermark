...
try:
    ...
    try:
        ...
        else: logging.info(f"galois BCH init mismatch."); _test_bch_ok = False; BCH_CODE_OBJECT = None
        if _test_bch_ok:
    ...
    else: logging.info("galois: Тесты не пройдены. ECC декодирование будет отключено.")
...
LAMBDA_PARAM: float = 0.1 # Не используется в экстракторе напрямую, но для консистентности
ALPHA_MIN: float = 1.005 # Используется в compute_adaptive_alpha_entropy
ALPHA_MAX: float = 1.12  # Используется в compute_adaptive_alpha_entropy
N_RINGS: int = 8
...
BCH_M: int = 8 # Параметры для проверки объекта BCH
BCH_T: int = 4 # Параметры для проверки объекта BCH
...
def extract_single_bit(f1:np.ndarray, f2:np.ndarray, ring_idx:int, n_rings:int=N_RINGS, fn:int=0, embed_comp:int=EMBED_COMPONENT) -> Optional[int]:
    """Извлекает один бит из заданного кольца пары кадров."""
    pair_index = fn // 2
    try:
        if f1 is None or f2 is None or f1.shape != f2.shape: return None
        # Конвертация в YCrCb и извлечение компонента
        try:
        ...
        L1=np.array(p1.lowpass); L2=np.array(p2.lowpass) # Получаем NumPy lowpass
        ...
        # Логирование (опционально вычисляем alpha для сравнения)
        alpha_for_log = compute_adaptive_alpha_entropy(rv1, ring_idx, fn)
        logging.debug(f"[P:{pair_index}, R:{ring_idx}] s1={s1:.4f}, s2={s2:.4f}, ratio={ratio:.4f} vs thr={threshold:.1f} (alpha_embd≈{alpha_for_log:.4f}) -> Bit={extracted_bit}")

        return extracted_bit
    except Exception as e:
        logging.error(f"Extract single bit failed (P:{pair_index}, R:{ring_idx}): {e}", exc_info=False) # Убрал exc_info
        return None

# --- Воркер для ОДНОЙ ПАРЫ (выбор колец + извлечение) ---
def _extract_single_pair_task(args: Dict[str, Any]) -> Tuple[int, List[Optional[int]]]:
    """Обрабатывает одну пару: выбирает кольца и извлекает биты."""
    ...
    extracted_bits: List[Optional[int]] = [None] * nrtu # Инициализируем список для бит
    try:
        # 1. Выбор колец (так же, как в эмбеддере)
        cands = get_fixed_pseudo_random_rings(pair_idx, nr, cps);
        if len(cands) < nrtu: raise ValueError("Not enough candidates")
        try: # Блок для ошибок при выборе колец
             comp1_sel = f1[:,:,ec].astype(np.float32)/255.;
             p1 = dtcwt_transform(comp1_sel, fn); assert p1 and p1.lowpass is not None, "DTCWT L1 failed"
             L1s = np.array(p1.lowpass); coords = ring_division(L1s, nr, fn);
             entropies=[]
             min_pixels_for_entropy = 10  # Перенес сюда для ясности
             for r_idx in cands:
                 e = -float('inf');
                 c = coords[r_idx];  # Получаем координаты для кандидата r_idx
                 if c is not None and c.shape[0] >= min_pixels_for_entropy:
                     # --- Исправленный блок try-except ---
                     try:
                         rs, cs = c[:, 0], c[:, 1];
                         rv = L1s[rs, cs];
                         se, _ = calculate_entropies(rv, fn, r_idx);  # Вызываем исправленную функцию
                         if np.isfinite(se):  # Проверяем, что энтропия - нормальное число
                             e = se
                     except Exception as entropy_e:  # Ловим ЛЮБУЮ ошибку при расчете
                         # Можно добавить логгирование ошибки, если нужно
                         # logging.warning(f"[P:{pair_idx}, R:{r_idx}] Entropy calculation failed: {entropy_e}")
             ...
             if len(final_rings) < nrtu: raise RuntimeError(
                 f"Not enough valid rings selected {len(final_rings)}<{nrtu}")
             logging.info(f"[P:{pair_idx}] Selected rings for extraction: {final_rings}")
        except Exception as sel_err: logging.error(f"[P:{pair_idx}] Ring selection error: {sel_err}"); return pair_idx, extracted_bits # Возвращаем None биты
    ...
    except Exception as e: logging.error(f"Error in single pair task P:{pair_idx}: {e}", exc_info=True); return pair_idx, extracted_bits # Возвращаем None биты
...
def extract_watermark_from_video(
    ...
    batch_size = max(1, ceil(num_valid_tasks / (num_workers * 2)))
