...
logging.info(
    f"--- Запуск Скрипта Извлечения (Bits/Pair: {BITS_PER_PAIR}, Payload: {PAYLOAD_LEN_BYTES * 8}bit, ECC Ожид.: {USE_ECC}, Доступно: {BCHLIB_AVAILABLE}, Max Repeats: {MAX_PACKET_REPEATS}) ---")
logging.info(
    f"Ожид. настройки эмбеддера: Метод='{RING_SELECTION_METHOD}', Метрика='{RING_SELECTION_METRIC}', Колец исп.={NUM_RINGS_TO_USE}, N_RINGS_Total={N_RINGS}")
...
def select_embedding_rings(
    ...
    func_start_time = time.time()
    metric_values: List[Tuple[float, int]] = []
    ...
    for i, coords_np in enumerate(rings_coords_np):
        ...
        try:
            if coords_np.ndim != 2 or coords_np.shape[1] != 2: logging.warning(
                f"[F:{frame_number}, R:{i}] Invalid coords shape."); metric_values.append((current_metric, i)); continue
            ...
            if not np.isfinite(current_metric): logging.warning(
                f"[F:{frame_number}, R:{i}] Metric is not finite ({current_metric}). Skipping."); current_metric = -float(
                'inf')
    ...
    if len(selected_indices) < num_to_select: logging.error(
...
def bits_to_bytes(bits: List[Optional[int]]) -> Optional[bytearray]:
    ...
    if remainder != 0:
        logging.warning(
            f"Number of valid bits ({len(valid_bits)}) not multiple of 8. Padding with {8 - remainder} zeros at the end.")
    ...
    for i in range(0, len(valid_bits), 8):
        ...
        try:
            ...
            logging.error(f"Error converting bits to byte: {byte_bits}");
            return None
        except TypeError:
            logging.error(f"Non-integer found in bits: {byte_bits}");
            return None  # Добавлено
    return byte_array
...
def decode_ecc(packet_bytes: bytearray, bch: 'bchlib.BCH', expected_data_len_bytes: int) -> Tuple[Optional[bytes], int]:
    ...
    if not BCHLIB_AVAILABLE or bch is None:
        ...
        if len(packet_bytes) >= expected_data_len_bytes:
            ...
            logging.error(
                f"Packet length ({len(packet_bytes)}) < expected data length ({expected_data_len_bytes}) without ECC.")
    ...
    k_bits = bch.k
    n_bits = bch.n
    expected_packet_len_bytes = ceil(n_bits / 8.0)  # Ожидаемая длина пакета

    current_packet_len_bytes = len(packet_bytes)
    if current_packet_len_bytes < expected_packet_len_bytes:
        logging.warning(
            f"decode_ecc: Packet length ({current_packet_len_bytes} bytes) < expected BCH block length ({expected_packet_len_bytes} bytes). Attempting decode.")
    ...
    if actual_data_bytes_len < data_bytes_len_expected:
        logging.warning(
            f"  Data part seems truncated ({actual_data_bytes_len} bytes < {data_bytes_len_expected} expected).")
    ...
    if len(ecc_to_decode) < ecc_bytes_len:
        ...
        logging.warning(
            f"decode_ecc: ECC part length {len(ecc_to_decode)} < expected {ecc_bytes_len}. Padding with {padding_needed} zeros.")
    ...
    if len(data_to_decode) < data_bytes_len_expected:
        ...
        logging.warning(
            f"decode_ecc: Data part length {len(data_to_decode)} < expected {data_bytes_len_expected}. Padding with {padding_needed} zeros.")
        data_to_decode.extend([0] * padding_needed)
    elif len(data_to_decode) > data_bytes_len_expected:
        logging.error(
            f"decode_ecc: Data part length {len(data_to_decode)} > expected {data_bytes_len_expected}. Truncating (Logic Error?).")
        data_to_decode = data_to_decode[:data_bytes_len_expected]

    try:
        data_copy = data_to_decode[:]
        ...
        if errors_corrected == -1:
            ...
            if len(corrected_data) >= expected_data_len_bytes:
                ...
                logging.error(
                    f"Internal error: Corrected data length ({len(corrected_data)}) < expected payload ({expected_data_len_bytes}).")
                return None, -1
...
def extract_frame_pair(
    ...
    try:
        if frame1 is None or frame2 is None: logging.error(
            f"[P:{pair_num_log}, R:{ring_index}] Input frame None."); return None
        if frame1.shape != frame2.shape: logging.error(
            f"[P:{pair_num_log}, R:{ring_index}] Frame shapes mismatch."); return None
        if frame1.dtype != np.uint8: frame1 = np.clip(frame1, 0, 255).astype(
            np.uint8);  # logging.warning(f"[P:{pair_num_log}] F1 type corrected.")
        if frame2.dtype != np.uint8: frame2 = np.clip(frame2, 0, 255).astype(
            np.uint8);  # logging.warning(f"[P:{pair_num_log}] F2 type corrected.")

        try:
            frame1_ycrcb = cv2.cvtColor(frame1, cv2.COLOR_BGR2YCrCb);
            frame2_ycrcb = cv2.cvtColor(frame2,
                                        cv2.COLOR_BGR2YCrCb)
        except cv2.error as e:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Color conversion failed: {e}");
            return None

        comp_name = ['Y', 'Cr', 'Cb'][
            embed_component];  # logging.debug(f"[P:{pair_num_log}, R:{ring_index}] Using {comp_name}")
        try:
            comp1 = frame1_ycrcb[:, :, embed_component].astype(np.float32) / 255.0;
            comp2 = frame2_ycrcb[:, :,
                    embed_component].astype(
                np.float32) / 255.0
        except IndexError:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Invalid component index.");
            return None
        ...
        if pyr1 is None or pyr2 is None or pyr1.lowpass is None or pyr2.lowpass is None:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] DTCWT failed.");
            return None
        L1 = pyr1.lowpass;
        L2 = pyr2.lowpass
        ...
        if not (0 <= ring_index < n_rings and ring_index < len(rings1_coords_np) and ring_index < len(
                rings2_coords_np)):
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Invalid ring_index {ring_index}.");
            return None
        ...
        if coords_1_np is None or coords_1_np.size == 0 or coords_2_np is None or coords_2_np.size == 0:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Ring {ring_index} coordinates invalid/empty.");
            return None
        if coords_1_np.ndim != 2 or coords_1_np.shape[1] != 2 or coords_2_np.ndim != 2 or coords_2_np.shape[1] != 2:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Invalid coords shape for ring.");
            return None
        ...
        try:
            ...
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] IndexError extracting ring values.",
                          exc_info=False);
            return None
        except Exception as e:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Error extracting ring values: {e}",
                          exc_info=False);
            return None

        if ring_vals_1.size == 0 or ring_vals_2.size == 0:
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] Extracted empty ring values.");
            return None
        ...
        try:
            ...
            logging.error(f"[P:{pair_num_log}, R:{ring_index}] SVD failed: {e}.");
            return None
        ...
        logging.info(
            f"[P:{pair_num_log}, R:{ring_index}] s1={s1:.4f}, s2={s2:.4f}, ratio={ratio:.4f} vs thr={threshold:.4f} -> Bit={bit_extracted}")
        ...
        logging.error(
            f"!!! UNHANDLED EXCEPTION in extract_frame_pair (Pair {pair_num_log_err}, Ring {ring_index}): {e}",
            exc_info=True)
        return None
...
def _extract_frame_pair_worker(args: Dict[str, Any]) -> Tuple[int, List[Optional[int]]]:
    ...
    try:
        ...
        if effective_use_saved_rings:
            if pair_idx in saved_rings_dict:
                ...
                if len(target_ring_indices) != num_rings_to_use:
                    logging.error(
                        f"[P:{pair_idx}] Saved rings count mismatch ({len(target_ring_indices)} vs {num_rings_to_use} expected). Using empty list.")
                    target_ring_indices = []
                else:
            ...
            try:
                # компонент и lowpass для выбора колец
                comp1_sel = frame1[:, :, embed_component].astype(np.float32) / 255.0
                pyr1_sel = dtcwt_transform(comp1_sel, frame_number=frame_number)
                if pyr1_sel is None or pyr1_sel.lowpass is None: raise RuntimeError(
                    "DTCWT failed for dynamic selection")
                ...
                if ring_selection_method == 'multi_ring':
                    ...
                    logging.error(
                        f"[P:{pair_idx}] Dynamic selection method '{ring_selection_method}' not fully supported for multi-bit in worker. Using empty list.")
                    target_ring_indices = []

                if len(target_ring_indices) < num_rings_to_use:
                    logging.warning(
                        f"[P:{pair_idx}] Dynamic selection failed to find {num_rings_to_use} rings. Found {len(target_ring_indices)}.")
        ...
        if target_ring_indices:
            ...
            # Если кольца не выбраны - список None
        ...
        if len(extracted_bits) != num_rings_to_use:
            logging.warning(
                f"[P:{pair_idx}] Worker returned list of unexpected length {len(extracted_bits)}. Padding/truncating to {num_rings_to_use}.")
...
def extract_watermark_from_video(
    ...
    logging.info(
        f"Starting parallel extraction (Bits/Pair: {bits_per_pair}) with packet voting (Max Repeats: {max_packet_repeats}).")
    logging.info(
        f"Ring Selection: {'Saved' if use_saved_rings else 'Dynamic (' + ring_selection_method + ')'}, Component: {['Y', 'Cr', 'Cb'][embed_component]}, Expect ECC: {use_ecc}")
    start_time = time.time()
    num_frames = len(frames);
    total_pairs_available = num_frames // 2
    processed_pairs_count = 0;
    failed_pair_extractions = 0

    if total_pairs_available == 0:
        logging.error("Not enough frames for any pairs.");
        return None

    # параметроы ECC и пакета
    bch = None
    packet_len_bits = payload_len_bytes * 8
    effective_use_ecc = False
    calculated_ecc_bits = 0
    bch_k_bits = payload_len_bytes * 8  # ардкод
    ...
    if use_ecc and BCHLIB_AVAILABLE:
        try:
            ...
            if payload_len_bytes * 8 > bch_k_bits:
                logging.warning(
                    f"Payload size ({payload_len_bytes * 8} bits) > BCH capacity k={bch_k_bits} bits! ECC might not work as expected.")
                packet_len_bits = (payload_len_bytes * 8) + calculated_ecc_bits
            else:
                packet_len_bits = bch_n_bits

            logging.info(
                f"BCH initialized (m={bch_m}, t={bch_t}). Payload={payload_len_bytes * 8}, ECC={calculated_ecc_bits}, k={bch_k_bits}, n={bch_n_bits}.")
            logging.info(
                f"===> Ожидаемая длина пакета (n): {packet_len_bits} бит ({ceil(packet_len_bits / 8.0)} байт).")
    ...
    saved_rings_dict: Dict[int, List[int]] = {};
    ...
    logging.info(
        f"Attempting to extract from {pairs_to_extract} pairs (up to {total_pairs_available} available, covers up to {max_packet_repeats} repeats).")
    ...
    if effective_use_saved_rings and not all(i in saved_rings_dict for i in range(pairs_to_extract)):
        ...
        logging.warning(
            f"Saved rings only available up to pair {max_pair_from_rings}. Reducing pairs to extract from {original_pairs_to_extract} to {pairs_to_extract}.")
    ...
    for i in range(pairs_to_extract):
        ...
        if idx2 >= num_frames or frames[idx1] is None or frames[idx2] is None:
            logging.error(f"Frames missing for pair {i}. Skipping.");
    ...
    logging.info(
        f"Submitting {len(tasks_args)} extraction tasks to {executor_class.__name__} (max_workers={MAX_WORKERS_EXTRACT})...")
    try:
        with executor_class(max_workers=MAX_WORKERS_EXTRACT) as executor:
            future_to_pair_idx = {executor.submit(_extract_frame_pair_worker, arg): arg['pair_idx'] for arg in
                                  tasks_args}
            for future in concurrent.futures.as_completed(future_to_pair_idx):
                pair_idx = future_to_pair_idx[future]
                try:
                    _, bits_result_list = future.result()
                    ...
                    logging.debug(
                        f"Pair {pair_idx} completed. Result: {bits_result_list} ({processed_pairs_count}/{len(tasks_args)})")
                except Exception as exc:
                    logging.error(f'Pair {pair_idx} generated exception: {exc}', exc_info=True)
                    extracted_bits_per_pair[pair_idx] = [None] * bits_per_pair
                    failed_pair_extractions += 1
                    processed_pairs_count += 1
    except Exception as e:
    ...
    logging.info(
        f"Parallel extraction finished. Pairs submitted: {len(tasks_args)}. Pairs processed: {processed_pairs_count}. Pairs with failed extraction (None bits): {failed_pair_extractions}.")

    extracted_bits_all: List[Optional[int]] = []
    total_extracted_bits_count = 0
    for i in range(pairs_to_extract):
        bits_list = extracted_bits_per_pair.get(i)
        if bits_list:  # Если для пары есть результат даже с нан
    ...
    logging.info(
        f"Total bits collected: {total_extracted_bits_count} (Expected based on pairs processed: {pairs_to_extract * bits_per_pair})")
    ...
    logging.info(
        f"Attempting to decode {num_potential_packets} potential packets (Packet Size: {packet_len_bits} bits)...")
    ...
    for i in range(num_potential_packets):
        ...
        if end_idx > total_extracted_bits_count:
            logging.warning(
                f"Not enough bits for full packet #{i + 1} (need {packet_len_bits}, have {total_extracted_bits_count - start_idx}). Skipping remaining.")
            break
        ...
        if len(packet_bytes) < expected_packet_len_bytes:
            logging.warning(
                f"Packet #{i + 1} byte length ({len(packet_bytes)}) < expected ({expected_packet_len_bytes}). Trying decode.")
        ...
        if payload is not None:
            logging.info(
                f"Successfully decoded payload from packet #{i + 1} (ECC corrected: {errors_corrected if errors_corrected >= 0 else 'N/A'}).")
    ...
    logging.info(
        f"Packet decoding summary: Success={decode_success_count}, Failed={decode_fail_count}. Total ECC corrections: {decode_ecc_corrected_total}.")
    ...
    for pld, count in payload_counts.most_common():
        try:
            ...
            logging.info(f"  Payload (Bytes: {pld}): {count} votes")
    ...
    logging.info(
        f"Most common payload selected with {winner_count}/{decode_success_count} votes (Confidence: {confidence:.1%}).")
...
def main():
    ...
    input_video_base = "watermarked_video_n2";
    ...
    if os.path.exists(ORIGINAL_WATERMARK_FILE):
        try:
            with open(ORIGINAL_WATERMARK_FILE, "r") as f:
                original_id_hex = f.read().strip()
            if original_id_hex and len(original_id_hex) == PAYLOAD_LEN_BYTES * 2:
                int(original_id_hex, 16)
                logging.info(f"Read original ID (Hex) from {ORIGINAL_WATERMARK_FILE}")
            else:
                logging.error(
                    f"Content of {ORIGINAL_WATERMARK_FILE} is not a valid hex ID of length {PAYLOAD_LEN_BYTES * 2}.")
                original_id_hex = None
        except ValueError:
            logging.error(f"Content of {ORIGINAL_WATERMARK_FILE} is not a valid hex string.")
            original_id_hex = None
        except Exception as e:
            logging.error(f"Error reading {ORIGINAL_WATERMARK_FILE}: {e}");
            original_id_hex = None
    else:
        logging.warning(f"{ORIGINAL_WATERMARK_FILE} not found.")

    logging.info("--- Starting Extraction Main Process ---")
    if not os.path.exists(input_video): logging.critical(f"Input video not found: '{input_video}'."); print(
    ...
    if extracted_payload_bytes is not None:
        if len(extracted_payload_bytes) == PAYLOAD_LEN_BYTES:
            ...
            logging.error(
                f"Extracted payload length mismatch ({len(extracted_payload_bytes)} vs {PAYLOAD_LEN_BYTES} bytes).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        profile_file = "profile_stats_extract_n2.txt"
        try:
