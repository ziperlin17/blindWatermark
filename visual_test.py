...
class TestWatermarkingStagesVisualization(unittest.TestCase):
    ...
    def test_02_compare_s1_values(self):
        ...
        for ring_idx in range(self.n_rings):
            ...
            # Обрабатываем только если оба значения s1 были успешно вычислены
            if s1_t is not None and s1_t1 is not None:
                ...
                bit_str = "N/A"
                # Используем немного большую дельту для проверки деления на ноль
                if abs(s1_t1) > 1e-12:
                    ratio = s1_t / s1_t1
                    ratios.append(ratio)
                    ratio_str = f"{ratio:.6f}"
                    # Предполагаемый бит (как в экстракторе)
                    extracted_bit = 0 if ratio >= 1.0 else 1
                    bit_str = str(extracted_bit)
                else:
                    ratios.append(np.nan) # Используем NaN для некорректных отношений
                ...
                # Логируем, если не удалось получить s1 для одного или обоих кадров
        ...
        # Проверка на близость s1 (если кадры похожи и знака нет)
        ...
        # Опциональный ассерт - может не пройти, если кадры сильно отличаются или есть знак
        # self.assertLess(mean_relative_diff, 0.2, "Относительная разница s1 между кадрами велика (>20%), проверьте кадры/наличие знака")

        # Игнорируем NaN при вычислении статистики отношений
        ...
        # Опциональный ассерт - отношение должно быть около 1.0 для немодифицированных кадров
        # self.assertAlmostEqual(mean_ratio, 1.0, delta=0.1, msg="Среднее отношение s1 сильно отличается от 1.0")
        ...
        if self.visualize and len(ring_indices_analyzed) > 0:
            ...
            plt.xticks(ring_indices_analyzed) # Показываем только индексы обработанных колец
            plt.yscale('log') # Логарифмическая шкала часто полезна для s1
            plt.grid(True, which="both", ls="--")
            plt.legend()

            # График отношений
            plt.subplot(1, 2, 2)
            valid_ratios_mask = ~np.isnan(ratios) # Маска для валидных отношений
            if np.any(valid_ratios_mask): # Рисуем только если есть валидные отношения
            ...
            # plt.yscale('log') # Лог шкала для отношения может быть менее информативна
            plt.grid(True)
            plt.legend()
...
if __name__ == '__main__':
    ...
    # Дополнительный вызов для отображения последнего графика, если тесты запускаются из IDE
    if plt.get_fignums():
