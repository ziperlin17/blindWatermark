# -*- coding: utf-8 -*-
# Файл: embedder.py (Версия с ECC, UUID, частичным покрытием - ИСПРАВЛЕНО v2)
...
try:
    import bchlib # Для ECC кодов
    BCHLIB_AVAILABLE = True
except ImportError:
    BCHLIB_AVAILABLE = False
    # Логирование ошибки произойдет при проверке в main
import cProfile
import pstats
...
USE_ECC: bool = True # Использовать ECC? (Требует bchlib)
BCH_M: int = 8
BCH_T: int = 5
...
logging.info(f"--- Запуск Скрипта Встраивания (ECC: {effective_use_ecc}, Покрытие: {COVERAGE_PERCENTAGE}%) ---")
...
if effective_use_ecc:
    logging.info(f"ECC Параметры: BCH m={BCH_M}, t={BCH_T}") # <-- Убрано обращение к bch.k, n, ecc_bits
...
# ============================================================
# (Функции dct_1d, idct_1d, dtcwt_transform, dtcwt_inverse,
...
# select_embedding_ring, calculate_perceptual_mask,
# calculate_spatial_weights_vectorized, payload_to_bits, add_ecc
# ОСТАЮТСЯ БЕЗ ИЗМЕНЕНИЙ, как в предыдущих ответах)
...
def calculate_perceptual_mask(input_plane: np.ndarray, frame_number: int = -1) -> Optional[np.ndarray]:
    if not isinstance(input_plane, np.ndarray) or input_plane.ndim != 2: logging.error(f"[F:{frame_number}] Invalid input for perceptual mask."); return None
    try:
        plane_32f = input_plane.astype(np.float32)
        gx = cv2.Sobel(plane_32f, cv2.CV_32F, 1, 0, ksize=3); gy = cv2.Sobel(plane_32f, cv2.CV_32F, 0, 1, ksize=3)
        grad_mag = np.sqrt(gx**2 + gy**2)
        local_brightness = cv2.GaussianBlur(plane_32f, (11, 11), 5); brightness_mask = 1.0 - np.abs(local_brightness - 0.5) * 2.0
        mean_sq = cv2.GaussianBlur(plane_32f**2, (11, 11), 5); sq_mean = local_brightness**2
        local_variance = np.maximum(mean_sq - sq_mean, 0); local_stddev = np.sqrt(local_variance)
        eps = 1e-9; max_grad = np.max(grad_mag); grad_norm = grad_mag / (max_grad + eps) if max_grad > eps else np.zeros_like(grad_mag)
        max_stddev = np.max(local_stddev); stddev_norm = local_stddev / (max_stddev + eps) if max_stddev > eps else np.zeros_like(local_stddev)
        ...
        max_mask = np.max(mask); mask_norm = mask / (max_mask + eps) if max_mask > eps else np.zeros_like(mask)
    ...
    except Exception as e: logging.error(f"[F:{frame_number}] Error calculating perceptual mask: {e}", exc_info=True); return np.ones_like(input_plane, dtype=np.float32)

def calculate_spatial_weights_vectorized(shape: Tuple[int, int], rows: np.ndarray, cols: np.ndarray) -> np.ndarray:
    h, w = shape; center_r, center_c = (h - 1) / 2.0, (w - 1) / 2.0
...
def payload_to_bits(payload_bytes: bytes) -> List[int]:
    bits = [];
    for byte in payload_bytes: bits.extend([int(b) for b in bin(byte)[2:].zfill(8)])
    return bits

def add_ecc(data_bytes: bytes, bch: 'bchlib.BCH') -> bytes:
    if not BCHLIB_AVAILABLE: logging.warning("bchlib не доступна, ECC не добавляется."); return data_bytes
    try:
        ecc = bch.encode(bytearray(data_bytes)); packet = bytearray(data_bytes) + ecc
        # Убрано обращение к bch.ecc_bytes, т.к. оно может отсутствовать
        logging.info(f"ECC: Добавлено {len(ecc)} байт ECC к {len(data_bytes)} байтам данных. Итого: {len(packet)} байт.")
        return bytes(packet)
    except Exception as e: logging.error(f"Ошибка при кодировании ECC: {e}", exc_info=True); return data_bytes
...
# --- АДАПТИРОВАННАЯ embed_frame_pair (Идентична предыдущему ответу) ---
def embed_frame_pair(
    ...
    # (Код embed_frame_pair - идентичен предыдущему ответу)
    func_start_time = time.time(); pair_num_log = frame_number // 2
    logging.debug(f"--- Embed Start: Pair {pair_num_log} (F:{frame_number}) ---")
    try:
        ...
        except cv2.error as e: logging.error(f"[P:{pair_num_log}] Color conversion failed."); return None, None, default_ring_index
        ...
        except IndexError: logging.error(f"[P:{pair_num_log}] Invalid component index."); return None, None, default_ring_index
        ...
        selected_rings_indices = []; current_ring_index = default_ring_index
        if ring_selection_method == 'deterministic': current_ring_index = deterministic_ring_selection(frame1_bgr, n_rings, frame_number=frame_number); selected_rings_indices = [current_ring_index]
        elif ring_selection_method == 'keypoint': current_ring_index = keypoint_based_ring_selection(frame1_bgr, n_rings, frame_number=frame_number); selected_rings_indices = [current_ring_index]
        elif ring_selection_method == 'multi_ring':
             metric_values = []; temp_metric = ring_selection_metric if ring_selection_metric in ['entropy', 'energy', 'variance', 'mean_abs_dev'] else 'entropy'
             for i_mr, coords_np_mr in enumerate(rings1_coords_np):
                 if coords_np_mr is not None and coords_np_mr.size > 0:
                     try:
                         rows_mr, cols_mr = coords_np_mr[:, 0], coords_np_mr[:, 1]; ring_vals_mr = L1[rows_mr, cols_mr].astype(np.float32)
             ...
             metric_values.sort(key=lambda x: x[0], reverse=True); selected_rings_indices = [idx for val, idx in metric_values[:num_rings_to_use] if val > -float('inf')]
             if not selected_rings_indices: selected_rings_indices = [DEFAULT_RING_INDEX]
             current_ring_index = selected_rings_indices[0]; logging.info(f"[P:{pair_num_log}] Multi-ring selected (metric '{temp_metric}'): {selected_rings_indices}")
        elif ring_selection_method == 'adaptive': current_ring_index = select_embedding_ring(L1, rings1_coords_np, metric=ring_selection_metric, frame_number=frame_number); selected_rings_indices = [current_ring_index]
        ...
        for ring_idx in selected_rings_indices:
            if 0 <= ring_idx < n_rings and ring_idx < len(rings1_coords_np) and rings1_coords_np[ring_idx] is not None and rings1_coords_np[ring_idx].size > 0 and ring_idx < len(rings2_coords_np) and rings2_coords_np[ring_idx] is not None and rings2_coords_np[ring_idx].size > 0: valid_selected_rings.append(ring_idx)
            else: logging.warning(f"[P:{pair_num_log}] Ring {ring_idx} invalid/empty. Removing.")
        if not valid_selected_rings:
            logging.error(f"[P:{pair_num_log}] No valid rings left! Trying default."); ring_idx = DEFAULT_RING_INDEX
            if 0 <= ring_idx < n_rings and ring_idx < len(rings1_coords_np) and rings1_coords_np[ring_idx] is not None and rings1_coords_np[ring_idx].size > 0 and ring_idx < len(rings2_coords_np) and rings2_coords_np[ring_idx] is not None and rings2_coords_np[ring_idx].size > 0:
                 ...
                 logging.warning(f"Default ring {DEFAULT_RING_INDEX} invalid. Searching..."); found_available = False
                 for idx in range(n_rings):
                      if idx < len(rings1_coords_np) and rings1_coords_np[idx] is not None and rings1_coords_np[idx].size > 0 and idx < len(rings2_coords_np) and rings2_coords_np[idx] is not None and rings2_coords_np[idx].size > 0:
                           valid_selected_rings = [idx]; current_ring_index = idx; logging.warning(f"Using first available ring {idx}."); found_available = True; break
                 if not found_available: logging.critical(f"[P:{pair_num_log}] All rings empty!"); return None, None, default_ring_index
        if current_ring_index not in valid_selected_rings: current_ring_index = valid_selected_rings[0] if valid_selected_rings else default_ring_index
        ...
        if use_perceptual_masking:
            ...
            elif perceptual_mask is None: logging.warning(f"[P:{pair_num_log}] Failed perceptual mask calculation.")
        for ring_idx in valid_selected_rings:
            try:
                coords_1_np = rings1_coords_np[ring_idx]; coords_2_np = rings2_coords_np[ring_idx]
                rows1, cols1 = coords_1_np[:, 0], coords_1_np[:, 1]; rows2, cols2 = coords_2_np[:, 0], coords_2_np[:, 1]
                ring_vals_1 = L1[rows1, cols1].astype(np.float32); ring_vals_2 = L2[rows2, cols2].astype(np.float32)
            except IndexError: logging.error(f"[P:{pair_num_log}, R:{ring_idx}] IndexError extracting vals.", exc_info=False); continue
            except Exception as e: logging.error(f"[P:{pair_num_log}, R:{ring_idx}] Error extracting vals: {e}", exc_info=False); continue
            if ring_vals_1.size == 0 or ring_vals_2.size == 0: logging.error(f"[P:{pair_num_log},R:{ring_idx}] Empty vals."); continue
            ...
            except np.linalg.LinAlgError as e: logging.error(f"[P:{pair_num_log},R:{ring_idx}] SVD failed: {e}."); continue
            ...
            log_lvl = logging.INFO if modified else logging.DEBUG; logging.log(log_lvl, f"[P:{pair_num_log},R:{ring_idx}] SVD Mod Applied: {modified}. Orig s1={s1:.4f}, s2={s2:.4f}. New s1={new_s1:.4f}, s2={new_s2:.4f}. Target bit={bit}.")
            ...
            if len(ring_vals_1_mod) == len(rows1):
                delta1 = ring_vals_1_mod - ring_vals_1; mod_factors1 = np.ones_like(delta1)
                if perceptual_mask is not None: mod_factors1 *= perceptual_mask[rows1, cols1]
            ...
            else: logging.error(f"[P:{pair_num_log}, R:{ring_idx}] Length mismatch L1"); continue
            if len(ring_vals_2_mod) == len(rows2):
                delta2 = ring_vals_2_mod - ring_vals_2; mod_factors2 = np.ones_like(delta2)
                if perceptual_mask is not None: mod_factors2 *= perceptual_mask[rows2, cols2]
            ...
            else: logging.error(f"[P:{pair_num_log}, R:{ring_idx}] Length mismatch L2"); continue
        ...
        if comp1_mod.shape != target_shape: comp1_mod = cv2.resize(comp1_mod, (target_shape[1], target_shape[0]), interpolation=cv2.INTER_LINEAR); logging.warning(f"[P:{pair_num_log}] Resized comp1 after inverse")
        if comp2_mod.shape != target_shape: comp2_mod = cv2.resize(comp2_mod, (target_shape[1], target_shape[0]), interpolation=cv2.INTER_LINEAR); logging.warning(f"[P:{pair_num_log}] Resized comp2 after inverse")
        ...
        new_ycrcb1 = np.stack((Y1_orig, Cr1, Cb1), axis=-1); new_ycrcb2 = np.stack((Y2_orig, Cr2, Cb2), axis=-1)
...
# --- АДАПТИРОВАННАЯ embed_watermark_in_video (для частичного покрытия) ---
def embed_watermark_in_video(
        frames: List[np.ndarray], watermark_bits: List[int],
        n_rings: int = N_RINGS,
    ...
    bits_per_packet = len(watermark_bits)
    if total_pairs == 0 or bits_per_packet == 0:
        logging.warning("Нет пар кадров или нулевая длина ВЗ. Встраивание не выполняется.")
        return frames[:]
    target_coverage = max(0.0, min(100.0, coverage_percentage))
    num_pairs_to_process = int(total_pairs * (target_coverage / 100.0))
    if num_pairs_to_process == 0 and target_coverage > 0: num_pairs_to_process = 1
    if num_pairs_to_process > total_pairs: num_pairs_to_process = total_pairs
    if num_pairs_to_process == 0: bits_for_embedding = []
    else: repeats = ceil(num_pairs_to_process / bits_per_packet) if bits_per_packet > 0 else 1; bits_for_embedding = (watermark_bits * repeats)[:num_pairs_to_process]
    logging.info(f"Starting embedding: {len(bits_for_embedding)} total bits across {num_pairs_to_process} frame pairs ({target_coverage:.1f}% coverage).")
    logging.info(f"Packet length (payload+ECC): {bits_per_packet} bits.")
    ...
    for pair_idx in range(num_pairs_to_process):
        ...
        if idx2 >= num_frames or frames[idx1] is None or frames[idx2] is None:
            logging.warning(f"Skipping pair {pair_idx} (Frames {idx1}, {idx2}) due to missing frames within coverage range.")
            continue
        args = {
            'idx1': idx1, 'frame1': frames[idx1], 'frame2': frames[idx2],
            'bit': bits_for_embedding[pair_idx],
            ...
            'frame_number': idx1, 'visualize_mask': visualize_masks,
            'use_perceptual_masking': use_perceptual_masking, 'embed_component': embed_component,
            'num_rings_to_use': num_rings_to_use
        }
        tasks_args.append(args)
    if not tasks_args: logging.error("No valid tasks created for embedding."); return watermarked_frames
    results: Dict[int, Tuple[Optional[np.ndarray], Optional[np.ndarray]]] = {}
    selected_rings_log: List[int] = [-1] * num_pairs_to_process
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_idx1)):
                ...
                try:
                    ...
                    else: logging.warning(f"Pair index {pair_num_log} out of bounds for selected_rings_log.")
                    if f1_mod is not None and f2_mod is not None:
                        results[idx1] = (f1_mod, f2_mod); processed_count += 1; logging.debug(f"Pair {pair_num_log} completed ({i + 1}/{task_count}). Ring: {selected_ring}")
                    else: error_count += 1; logging.error(f"Pair {pair_num_log} (Frame {idx1}) failed (returned None).")
                except Exception as exc: error_count += 1; logging.error(f'Pair {pair_num_log} (Frame {idx1}) generated exception: {exc}', exc_info=True);
    ...
    logging.info(f"Applied results from {len(results)} pairs to {update_count} frames.")
    final_selected_rings = [r for i, r in enumerate(selected_rings_log) if i < num_pairs_to_process and r >= 0]
    if final_selected_rings:
        try:
            with open(SELECTED_RINGS_FILE, 'w') as f: json.dump(final_selected_rings, f, indent=4)
        ...
        except Exception as e: logging.error(f"Unexpected error saving rings JSON: {e}", exc_info=True)
    else: logging.warning("No valid selected rings recorded to save.")
...
# ============================================================
def main():
    # (Код main с ИСПРАВЛЕНИЕМ для UnboundLocalError и AttributeError)
    main_start_time = time.time()
    input_video = "input.mp4"; base_output_name = "watermarked_video"; output_video = base_output_name + OUTPUT_EXTENSION
    ...
    original_uuid = uuid.uuid4(); original_uuid_str = str(original_uuid)
    payload_bytes = original_uuid.bytes
    logging.info(f"Generated Payload UUID: {original_uuid_str}")
    logging.info(f"Payload size: {len(payload_bytes)} bytes ({len(payload_bytes)*8} bits)")

    final_payload_bytes = payload_bytes; bch = None; local_use_ecc = USE_ECC and BCHLIB_AVAILABLE
    if local_use_ecc:
        try:
            bch = bchlib.BCH(m=BCH_M, t=BCH_T)
            # Убрали логирование атрибутов k, n, ecc_bits, которые могут отсутствовать
            logging.info(f"BCH initialized (m={BCH_M}, t={BCH_T}).")
            final_payload_bytes = add_ecc(payload_bytes, bch)
        except Exception as e:
            ...
            final_payload_bytes = payload_bytes # Используем исходные данные

    final_watermark_bits = payload_to_bits(final_payload_bytes)
    logging.info(f"Total bits per packet to embed (Payload + {'ECC' if local_use_ecc else 'No ECC'}): {len(final_watermark_bits)}")

    try:
        with open(ORIGINAL_WATERMARK_FILE, "w") as f: f.write(original_uuid_str)
        logging.info(f"Original UUID saved to {ORIGINAL_WATERMARK_FILE}")
    except IOError as e: logging.error(f"Could not save original UUID: {e}")

    enable_mask_visualization = False
    watermarked_frames = embed_watermark_in_video(
        frames=frames, watermark_bits=final_watermark_bits, n_rings=N_RINGS,
        ring_selection_method=RING_SELECTION_METHOD, ring_selection_metric=RING_SELECTION_METRIC,
        default_ring_index=DEFAULT_RING_INDEX, fps=fps_to_use, max_workers=MAX_WORKERS,
        visualize_masks=enable_mask_visualization, use_perceptual_masking=USE_PERCEPTUAL_MASKING,
    ...
    if watermarked_frames and len(watermarked_frames) == len(frames):
        ...
        try:
            if os.path.exists(output_video):
                ...
                if file_size_mb > 100 and OUTPUT_CODEC in ['HFYU', 'FFV1']: logging.warning(f"Large file size due to lossless codec '{OUTPUT_CODEC}'.")
                elif file_size_mb < 1 and OUTPUT_CODEC in ['XVID', 'mp4v']: logging.warning(f"Small file size with lossy codec '{OUTPUT_CODEC}'. Check quality/robustness.")
    ...
    print(f"Watermark (Original UUID): {ORIGINAL_WATERMARK_FILE}")
    print(f"Rings Log: {SELECTED_RINGS_FILE}")
    print("\nIMPORTANT: Please run the extractor and check the results.")
    if OUTPUT_CODEC in ['XVID', 'mp4v', 'MJPG']: print(f"Since lossy codec '{OUTPUT_CODEC}' was used, robustness depends on ALPHA values.")
...
if __name__ == "__main__":
    if USE_ECC and not BCHLIB_AVAILABLE:
        ...
        # Не меняем USE_ECC глобально, main обработает это через effective_use_ecc
    ...
    try:
        ...
        except IOError as e: logging.error(f"Could not save profiling stats to {profile_file}: {e}")