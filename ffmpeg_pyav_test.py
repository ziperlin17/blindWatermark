...
def check_hw_accel_support():
    ...
    for api, name in encoders:
        try:
            codec = av.codec.Codec(name, mode='w') # Пытаемся получить кодер
        ...
        except (av.FFmpegError, ValueError): # Ловим ошибку, если кодек не найден
    ...
    # --- Проверка Декодеров ---
    decoders = [
        ('nvenc', 'h264_cuvid'), ('nvenc', 'hevc_cuvid'), # NVDEC (cuvid)
        ('qsv', 'h264_qsv'), ('qsv', 'hevc_qsv'),
        ('vaapi', 'h264_vaapi'), ('vaapi', 'hevc_vaapi'),
        ('amf', 'h264_amf'), ('amf', 'hevc_amf'), # AMF/VAAPI могут использоваться и для декодирования
    ...
    for api, name in decoders:
        try:
            codec = av.codec.Codec(name, mode='r') # Пытаемся получить декодер
    ...
    for api, support in hw_support.items():
        ...
        # Вывод декодера для dxva2/d3d11va
    ...
    # Дополнительная проверка через get_hw_config (если доступно)
    try:
         h264_codec = av.codec.Codec('h264', 'r') # Берем обычный h264 декодер
...
if __name__ == "__main__":
    # Настраиваем базовое логирование для проверки
