# Файл: test_scaling.py (Версия с 4 тестами: original, simplified, minimal, batched)
...
# --- Попытка импорта функций и констант из embedder ---
try:
    from watermark_embedder import (
        _embed_frame_pair_worker, # Оригинальный воркер
        embed_frame_pair,         # Используется для проверки логики
        dtcwt_transform,
        ring_division,
        calculate_entropies,
...
# --- Фикстура для ОРИГИНАЛЬНОГО воркера ---
...
# --- Фикстура для УПРОЩЕННОГО воркера ---
@pytest.fixture(scope="module")
def worker_test_data_simplified():
    ...
    # Выбираем кольца заранее (для упрощения передачи данных в тесте)
...
# --- Фикстура для МИНИМАЛЬНОГО воркера ---
@pytest.fixture(scope="module")
...
# --- Фикстура для БАТЧЕЙ ---
@pytest.fixture(scope="module")
def worker_test_data_batched(worker_test_data_original): # Зависит от оригинальных данных
    ...
    batch_size = max(1, (os.cpu_count() or 4) // 2) # Попробуем половину ядер в батче
    # ИЛИ можно фиксированный: batch_size = 10
...
# --- Вспомогательная функция для установки переменных окружения ---
...
# --- Вспомогательная функция для запуска и измерения времени ---
def run_and_measure(worker_func, args_list, num_workers, run_parallel=True, test_name=""):
    ...
    try:
        if run_parallel:
            ...
            # Убедимся, что num_workers > 0
            actual_workers = max(1, num_workers)
            with ProcessPoolExecutor(max_workers=actual_workers) as executor:
                # print(f"  Отправка задач в пул...")
                for args in args_list:
                    futures.append(executor.submit(worker_func, args))
                # print(f"  Ожидание завершения...")
                # Использование as_completed для получения результатов по мере готовности
                # и возможного вывода прогресса или отладки
                temp_results = {} # Словарь для сбора результатов по индексу/ключу
                for i, future in enumerate(as_completed(futures)):
                    # print(f"  Задача {i+1}/{num_tasks} завершена...") # Можно раскомментировать для отладки
                    try:
                         # В батчевом воркере результат - это список, в остальных - кортеж
                         res = future.result()
                         # Определяем ключ для сохранения результата
                         if isinstance(args_list[0], dict) and 'pair_idx' in args_list[0]:
                              key = args_list[i]['pair_idx'] # Для тестов original, simplified, minimal
                         elif isinstance(args_list[0], list): # Для батчевого теста
                              # Используем индекс батча как ключ
                              key = i
                         else: # Неожиданный формат аргументов
                              key = i
                         temp_results[key] = res
                    except Exception as e:
                         print(f"  !!! Ошибка при получении результата задачи {i}: {e}")
                         # Можно добавить обработку ошибки, например, записать None
                         key = i # Используем индекс как ключ для ошибки
                         temp_results[key] = f"ERROR: {e}"


            # Собираем финальные результаты в нужном порядке (если возможно)
            ...
            # Проверка, что все результаты получены (хотя бы в виде ошибки)
            if len(results) != num_tasks:
                 print(f"ПРЕДУПРЕЖДЕНИЕ: Получено {len(results)} результатов, ожидалось {num_tasks}")
                 # raise RuntimeError(f"Получено {len(results)} результатов, ожидалось {num_tasks}")

        else: # Последовательный запуск
            for args in args_list:
                results.append(worker_func(args)) # Прямой вызов
    except Exception as e:
    ...
    # Можно добавить проверку результатов, если нужно
    # print(f"   Пример результата: {results[0] if results else 'N/A'}")
    return elapsed_time, results


# --- Тестовый УПРОЩЕННЫЙ Воркер ---
def _test_worker_simplified(args_simplified: Dict):
    ...
    try:
        # Выполняем основные операции, которые могут быть узким местом
        ...
        # ring_division(pyramid1.lowpass, n_rings_arg, frame_number) # Можно добавить для нагрузки

        time.sleep(0.005) # Имитация остальной работы
        return pair_idx, True
    except Exception:
        return pair_idx, False

# --- Тестовый МИНИМАЛЬНЫЙ Воркер ---
def _test_worker_minimal(args_minimal: Dict):
    """Воркер, который почти ничего не делает."""
    pair_idx = args_minimal['pair_idx']
    time.sleep(0.005) # Имитация минимальной работы
    return pair_idx, True

# --- Тестовый Воркер для БАТЧЕЙ ---
def _test_worker_batch(batch_args_list: List[Dict]):
    ...
    if not EMBEDDER_IMPORTED or _embed_frame_pair_worker is None:
        return [] # Пропуск, если оригинал не импортирован

    batch_results = []
    # pid = os.getpid() # Для отладки
    # print(f"[PID:{pid}] Batch Start ({len(batch_args_list)} tasks)")
...
def run_scaling_test(worker_func, test_data, test_name, is_batch_test=False):
    ...
    set_thread_limits() # Устанавливаем лимиты потоков перед каждым тестом

    # Если это батчевый тест, test_data - это список батчей
    # Если не батчевый, test_data - это список аргументов для задач
    num_tasks_or_batches = len(test_data)
    if is_batch_test:
         # Общее количество задач = сумма длин всех батчей
         num_total_tasks = sum(len(batch) for batch in test_data)
         # Количество воркеров = количество батчей (но не больше ядер CPU)
         ...
         # Количество воркеров = количество задач (но не больше ядер CPU)
    ...
    # Нормируем время на задачу для сравнения
    ...
    # Идеальное параллельное время = последовательное / кол-во воркеров
    ideal_concurrent_time = sequential_time / num_workers if num_workers > 0 else sequential_time
    # Допуск на накладные расходы
    ...
    if num_workers > 1 and num_total_tasks > 1:
        # Основная проверка: параллельное время должно быть ниже порога
        ...
        # Дополнительная проверка: параллельное должно быть быстрее последовательного
        # (Даем небольшой запас на случай очень коротких задач и флуктуаций)
...
def test_scaling_batched(worker_test_data_batched):
    ...
    print("\n\n=== ЗАПУСК ТЕСТА БАТЧЕЙ ===")
    # Передаем батч-воркер и данные в батчах, указываем флаг is_batch_test=True
