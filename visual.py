# Файл: check_opencl.py
import time
import numpy as np
import logging
import sys # Для вывода информации об ошибке
...
# 2. Поиск платформ и устройств OpenCL
platforms = []
devices = []
...
# 3. Попытка создать контекст и очередь команд (если есть устройства)
context = None
queue = None
if devices: # Если мы нашли хотя бы одно устройство
    ...
    try:
        # Создаем контекст для этого устройства
        context = cl.Context([selected_device])
        # Создаем очередь команд для выполнения операций
...
# 4. Попытка выполнить простое вычисление (сложение векторов)
computation_ok = False
if context and queue: # Если контекст и очередь созданы
    logging.info("--- Попытка выполнить тестовое вычисление OpenCL (сложение векторов) ---")
    try:
        vector_size = 1024 * 1024 # Размер векторов для теста
        # Создаем данные на хосте (CPU)
        ...
        # Создаем буферы на устройстве OpenCL (GPU/CPU)
        # MF_READ_ONLY/MF_WRITE_ONLY - подсказки для оптимизации
        # MF_COPY_HOST_PTR - копируем данные с хоста при создании буфера
        mf = cl.mem_flags
        ...
        # Определяем простое OpenCL ядро (kernel) для сложения
        kernel_code = """
        ...
        # Запускаем ядро
        # program.vector_add(queue, host_a.shape, None, device_a, device_b, device_result)
        # или через атрибут:
        ...
        # Проверяем корректность результата (сравнение с NumPy)
